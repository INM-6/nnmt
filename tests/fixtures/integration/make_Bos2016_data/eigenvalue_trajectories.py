...
def plot_fig(calcAna, calcAnaAll, figname=None):
    ...
    ax[1].set_position([box.x0, box.y0 - box.height, box.width,
                        box.height * 2])
    ...
    for k, l in enumerate(ph.layers):
        ...
        for i in range(0, N, 3):
            j = k * 2 + i
            if i == 0:
                ax[2].plot(eigs[i, 2 * k: 2 * k + 2].real,
                           eigs[i, 2 * k: 2 * k + 2].imag,
    ...
    for k, l in enumerate(ph.layers):
        # plot every third eigenvalue
        for i in range(0, N, 3):
            ax[3].plot(eigs[i, 2 * k: 2 * k + 2].real,
                       eigs[i, 2 * k: 2 * k + 2].imag,
                       '.', color=get_color(i, l), markersize=1.0)
    # frequencies where eigenvalue trajectory is closest to one
    ...
    ax[3].set_position([box.x0, box.y0 - box.height, box.width,
                        box.height * 2])
