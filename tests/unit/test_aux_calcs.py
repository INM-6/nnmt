...
class Test_Phi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.expected_results = np.load('tests/unit/fixtures/phi.npy')

    def test_correct_predictions(self):
        
        result = Phi(self.test_inputs)
        np.testing.assert_almost_equal(result, self.expected_results, 5)
...
class Test_Phi_prime_mu(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # fixtures created with parameters above
        self.expected_results = np.load('tests/unit/fixtures/phi_prime_mu.npy')
...
class TestFiringRateDerivativeFunctions(TestFiringRateFunctions):
    ...
    def expected_firing_rate(self):
        ...
        expected_results = np.load(self.fixture)
    ...
    def test_correct_firing_rate_prediction(self):
        ...
        for expected_result, params in zip(self.expected_firing_rate, self.parameters_for_firing_rate_test):
...
class Test_d_nu_d_mu(unittest.TestCase, TestFiringRateDerivativeFunctions):
    ...
    def setUpClass(cls):
        ...
        mus = [3.30031035, 7.02709379, 7.18151477, 9.18259078] * ureg.mV
        sigmas = [6.1901737, 5.11420662, 5.96478947, 4.89397196] * ureg.mV
        cls._parameters_for_firing_rate_test_0 = [dict(mu=mu, sigma=sigma) for mu, sigma in zip(mus, sigmas)]
