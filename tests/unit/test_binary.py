...
class Test_mean_activitys_wrapper:

    func = staticmethod(binary.mean_activitys)

    def mock_mean_activity_integration(self, mocker):
        mocker.patch(
            'nnmt._solvers._mean_activity_integration',
            return_value=1
            )

    def test_raise_exception_if_not_all_parameters_available(self, mocker,
                                                             empty_network):
        self.mock_mean_activity_integration(mocker)
        with pytest.raises(RuntimeError):
            self.func(empty_network)


class Test_mean_activitys_for_given_input:

    func = staticmethod(binary._mean_activitys_for_given_input)
    fixtures = 'binary_mean_activitys.h5'
    rtol = 1e-7
